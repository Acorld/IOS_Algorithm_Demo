...
- (void)GetSmallerNumbers:(NSMutableArray *)array
{
    ...
    for (int i = array.count - 1; i >= array.count - kMaxNumbers; i--)  //遍历小顶推
    {
...
- (void)MinHeapify:(NSMutableArray *)array :(int)currentIndex :(int) heapSize
{
    ...
    if (leftChildIndex < heapSize && [array[leftChildIndex] isLargerThan:array[smallestIndex]])
    {
        smallestIndex = leftChildIndex;
    }
    if (rightChildIndex < heapSize && [array[rightChildIndex] isLargerThan:array[smallestIndex]])
    {
